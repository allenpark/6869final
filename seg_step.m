...
if compi ~= compj
    ...
    w = e(1);
    [mint, c] = MInt(compi, compj, num_comp, seg, im);
    stat = [vi vj w mint Int(compi, seg, im) Int(compj, seg, im) length(seg{compi, 1}) length(seg{compj, 1}) c];
    ...
    if w <= mint || (false && size(seg{larger_comp, 1}, 2) / size(seg{smaller_comp, 1}, 2) >= 3 && abs(Int(smaller_comp, seg, im) - w) < 20/256)
